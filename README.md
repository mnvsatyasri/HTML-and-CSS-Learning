...
### PROJECT - Controls (HTML + CSS)

#### Algorithm
1. Create an HTML file named `buttons.html`.
2. Link a CSS file named `buttons.css` inside the `<head>` section.
3. Add three `<button>` elements with different classes:
   - `subscribe-button`
   - `join-button`
   - `tweet-button`
4. In `buttons.css`, style each button with properties like:
   - Background color
   - Text color
   ...
   - Cursor type
   - Margins
5. Save both files and open `buttons.html` in a browser.
6. End — styled buttons displayed successfully.
...
# PROJECT - Ride Request Form (HTML + CSS)
...
3. Add the structural elements for the form:
   ...
   o Two <button> elements with different classes:
     - request-now-button
     - schedule-later-button
4. In ride.css , style each element with properties like:
   o Display: block
   o Background color
   o Text color
